...
export function activateModalFocusTrap(overlay, { dialogSelector = '[role="dialog"]', onClose } = {}) {
  if (!overlay) return () => {};
  ...
  const getFocusable = () => [...dialog.querySelectorAll(FOCUSABLE_SELECTOR)]
    .filter((node) => node instanceof HTMLElement && !node.hasAttribute('disabled') && node.offsetParent !== null);
...
export function deactivateModalFocusTrap(overlay) {
  overlay?.__modalCleanup?.();
