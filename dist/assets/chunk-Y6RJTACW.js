...
// src/builder.js
var LUCKY_ADJECTIVES = ["Cosmic", "Neon", "Velvet", "Ember", "Frozen", "Solar", "Midnight", "Crystal", "Thunder", "Phantom", "Ruby", "Jade", "Amber", "Silver", "Golden", "Copper", "Cobalt", "Crimson", "Indigo", "Scarlet"];
var LUCKY_NOUNS = ["Horizon", "Circuit", "Drift", "Pulse", "Aurora", "Nebula", "Prism", "Forge", "Cascade", "Vertex", "Bloom", "Cipher", "Wave", "Storm", "Spark", "Flame", "Shade", "Frost", "Tide", "Glow"];
...
async function maybeShowBuilderSubmitPrompt() {
  ...
  const { showInlineSignInPrompt } = await import("./chunk-W3BZNAVJ.js");
...
function resetBuilder() {
  ...
  setBuilderColors(getDefaultBuilderColors());
...
function colorMeLuckyVariant(variant) {
  builderColors.variant = variant;
  colorMeLucky();
  if (isMobile()) {
    import("./chunk-QGJ2PMPE.js").then((m) => m.mobileSetView("create"));
  }
  setTimeout(() => {
...
function toggleBuilderMode() {
  trackEvent("builder_toggled", null, { mode: panelMode === "builder" ? "closing" : "opening" });
  ...
  if (panelMode === "builder") {
    ...
    setBuilderColors(loadBuilderState() || getDefaultBuilderColors());
...
async function openBuilderForVariant(themeId, variant) {
...
function shareBuilderOnX() {
  ...
  const text = `I just created a "${name}" theme for Codex with @DexThemes! \u{1F3A8}
