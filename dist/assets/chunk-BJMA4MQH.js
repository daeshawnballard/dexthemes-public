  ...
  try {
    ...
    if (res.ok) {
      ...
      if (data.unlocked) {
        ...
        const { renderAuthUI } = await import("./chunk-3RFEH43Q.js");
        renderAuthUI();
        if (window.innerWidth <= 1024) {
          const { renderMobileBrowse } = await import("./chunk-P26T4MU3.js");
          renderMobileBrowse();
        }
...
async function recordSecretInteraction(activity) {
  ...
  try {
    ...
    if (window.innerWidth <= 1024) {
      ...
      const { renderSidebar } = await import("./chunk-7NNF7BXQ.js");
      renderSidebar();
    }
