...
function mobileStartSubmit() {
  const overlay = document.createElement("div");
  overlay.className = "mobile-submit-overlay";
  overlay.onclick = (e) => {
    if (e.target === overlay) dismissMobileSubmitModal();
  };
  overlay.innerHTML = `
    <div class="mobile-submit-modal">
      ...
      <input type="text" class="mobile-submit-name" id="mobile-submit-name"
             placeholder="Name your theme..." value="${escapeHtml(builderColors?.name || "")}" aria-label="Theme name" autofocus>
      ${currentUser ? `
...
function mobileDoSubmit() {
  ...
  if (nameInput && nameInput.value.trim()) {
    ...
    import("./chunk-PAIQCA7E.js").then((m) => {
